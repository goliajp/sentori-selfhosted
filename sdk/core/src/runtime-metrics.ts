...
export class RuntimeMetricBuffer {
  ...
  private buf: RuntimeMetricPoint[] = []
  private dropped = 0

  constructor(cap: number = RING_CAP) {
    this.cap = cap
  }
...
function validatePoint(name: string, value: number, tags?: Record<string, string>): boolean {
  ...
  if (tags) {
    ...
    if (keys.length > MAX_TAG_KEYS) return false
    for (const k of keys) {
      if (k.length === 0 || k.length > MAX_TAG_KEY_LEN) return false
      const v = tags[k]
      if (typeof v !== 'string' || v.length > MAX_TAG_VALUE_LEN) return false
    }
  }
  return true
}
...
export function emitMetric(
  ...
  if (!validatePoint(name, value, tags)) return
  _global.push({
    name,
    value,
    tags,
 ...
 *      }
 */
export function drainRuntimeMetricsForFlush(): RuntimeMetricPoint[] {
 ...
 *  preserve memory. */
export function rebufferRuntimeMetrics(points: RuntimeMetricPoint[]): void {
...
export function __peekRuntimeMetricsSize(): number {
  return _global.size()
}
