...
async function hashOne(keyType, raw) {
    if (raw == null || raw === '')
        return null;
    ...
    if (!subtle) {
        throw new Error('sentori: crypto.subtle unavailable; identity hashing requires WebCrypto');
    }
 ...
 *  generic trim. */
function normalise(keyType, raw) {
    switch (keyType) {
        case 'email':
 ...
 * sending raw).
 */
export async function hashIdentities(linkBy) {
