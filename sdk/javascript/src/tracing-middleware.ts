...
import { startSpan, type SpanContextLike, withSpan } from '@goliapkg/sentori-core'
...
export function parseTraceparent(header?: null | string): null | SpanContextLike {
  if (!header) return null
  const parts = header.trim().split('-')
  if (parts.length !== 4 || parts[0] !== '00') return null
  const trace = parts[1] ?? ''
  const parent = parts[2] ?? ''
  if (trace.length !== 32 || parent.length !== 16) return null
  if (!/^[0-9a-f]+$/i.test(trace) || !/^[0-9a-f]+$/i.test(parent)) return null
  const traceId =
    trace.slice(0, 8) + '-' + trace.slice(8, 12) + '-' +
    trace.slice(12, 16) + '-' + trace.slice(16, 20) + '-' +
    trace.slice(20, 32)
  const padded = parent + '0'.repeat(16)
  const spanId =
    padded.slice(0, 8) + '-' + padded.slice(8, 12) + '-' +
    padded.slice(12, 16) + '-' + padded.slice(16, 20) + '-' +
    padded.slice(20, 32)
  return { spanId: spanId.toLowerCase(), traceId: traceId.toLowerCase() }
}

// Minimal type shapes — we don't import from express / hono / fastify
// (they're optional peers that the consumer brings). Each interface
// captures only the methods we actually call.
...
export function expressTracingMiddleware() {
  return (req: ExpressLikeReq, res: ExpressLikeRes, next: ExpressNext): void => {
...
// Hono types (just enough for the middleware).
...
export function honoTracingMiddleware() {
  return async (c: HonoLikeContext, next: HonoNext): Promise<void> => {
...
// Fastify hook surface. Fastify lets you attach onRequest +
// onResponse hooks per-instance; we register both as one
...
type FastifyInstance = {
  addHook(
    name: 'onRequest',
    handler: (req: FastifyRequest, _reply: FastifyReply, done: () => void) => void,
  ): unknown
  addHook(
    name: 'onResponse',
 ...
 */
export function installFastifyTracing(fastify: FastifyInstance): void {
