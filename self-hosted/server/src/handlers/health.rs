...
///
/// Served by the server rather than dropped in the SPA's static
/// directory so that its `info.version` is the version of the binary
/// answering — a document that describes a different build than the
/// one you are talking to is the same failure as documentation that
/// describes a different product.
///
/// Generated by `scripts/gen-openapi.mjs` from the router; the check
/// mode of that script runs in preflight.
