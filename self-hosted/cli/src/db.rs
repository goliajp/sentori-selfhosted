//! The one place this binary opens a database connection.
//!
...
//!
//! This is a copy of `self-hosted/server/src/db.rs`. The CLI is a
//! standalone Cargo workspace on purpose — its Docker layer builds
//! without `core/` — so there is nowhere to share it from.
//! `scripts/check-timezone-pinned.mjs` requires the two to agree.
